  ...
  >
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div class="text-center">
          <VSidebar variant="default" class="mb-2" />
          <p class="text-sm text-gray-600">default</p>
        </div>
        <div class="text-center">
          <VSidebar variant="elevated" class="mb-2" />
          <p class="text-sm text-gray-600">elevated</p>
        </div>
        <div class="text-center">
          <VSidebar variant="transparent" class="mb-2" />
          <p class="text-sm text-gray-600">transparent</p>
        </div>
      </div>
    </div>
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div class="text-center">
          <VSidebar color="primary" class="mb-2" />
          <p class="text-sm text-gray-600">primary</p>
        </div>
        <div class="text-center">
          <VSidebar color="secondary" class="mb-2" />
          <p class="text-sm text-gray-600">secondary</p>
        </div>
        <div class="text-center">
          <VSidebar color="success" class="mb-2" />
          <p class="text-sm text-gray-600">success</p>
        </div>
        <div class="text-center">
          <VSidebar color="warning" class="mb-2" />
          <p class="text-sm text-gray-600">warning</p>
        </div>
        <div class="text-center">
          <VSidebar color="error" class="mb-2" />
          <p class="text-sm text-gray-600">error</p>
        </div>
        <div class="text-center">
          <VSidebar color="info" class="mb-2" />
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <VSidebar label="정상 상태" />
        <VSidebar label="비활성화" disabled />
        <VSidebar label="로딩 상태" loading />
        <VSidebar label="에러 상태" :error="true" />
      </div>
    </div>

    <!-- 인터랙티브 데모 -->
    <template #interactive>
      <InteractiveDemo :props="demoProps" component-name="VSidebar">
        <template #controls>
          <div class="space-y-4">
            <div>
...
const SidebarProps = [
  { name: 'label', type: 'string', default: '', description: 'Sidebar의 라벨' },
  { name: 'color', type: 'string', default: 'primary', description: 'Sidebar의 색상' },
  { name: 'size', type: 'string', default: 'default', description: 'Sidebar의 크기' },
...
const codeExample = 'VSidebar 컴포넌트 사용 예시'
</script>
