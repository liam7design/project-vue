  ...
  >
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center">
          <VLayout variant="default" class="mb-2" />
          <p class="text-sm text-gray-600">default</p>
        </div>
        <div class="text-center">
          <VLayout variant="sidebar" class="mb-2" />
          <p class="text-sm text-gray-600">sidebar</p>
        </div>
        <div class="text-center">
          <VLayout variant="header" class="mb-2" />
          <p class="text-sm text-gray-600">header</p>
        </div>
        <div class="text-center">
          <VLayout variant="footer" class="mb-2" />
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div class="text-center">
          <VLayout color="primary" class="mb-2" />
          <p class="text-sm text-gray-600">primary</p>
        </div>
        <div class="text-center">
          <VLayout color="secondary" class="mb-2" />
          <p class="text-sm text-gray-600">secondary</p>
        </div>
        <div class="text-center">
          <VLayout color="success" class="mb-2" />
          <p class="text-sm text-gray-600">success</p>
        </div>
        <div class="text-center">
          <VLayout color="warning" class="mb-2" />
          <p class="text-sm text-gray-600">warning</p>
        </div>
        <div class="text-center">
          <VLayout color="error" class="mb-2" />
          <p class="text-sm text-gray-600">error</p>
        </div>
        <div class="text-center">
          <VLayout color="info" class="mb-2" />
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <VLayout label="정상 상태" />
        <VLayout label="비활성화" disabled />
        <VLayout label="로딩 상태" loading />
        <VLayout label="에러 상태" :error="true" />
      </div>
    </div>

    <!-- 인터랙티브 데모 -->
    <template #interactive>
      <InteractiveDemo :props="demoProps" component-name="VLayout">
        <template #controls>
          <div class="space-y-4">
            <div>
...
import { ref } from 'vue'
import { VLayout } from '@/components/ui'
...
const LayoutProps = [
  { name: 'label', type: 'string', default: '', description: 'Layout의 라벨' },
  { name: 'color', type: 'string', default: 'primary', description: 'Layout의 색상' },
  { name: 'size', type: 'string', default: 'default', description: 'Layout의 크기' },
...
const codeExample = 'VLayout 컴포넌트 사용 예시'
</script>
