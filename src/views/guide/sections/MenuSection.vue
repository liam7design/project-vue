  ...
  >
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div class="text-center">
          <VMenu variant="light" class="mb-2" />
          <p class="text-sm text-gray-600">light</p>
        </div>
        <div class="text-center">
          <VMenu variant="dark" class="mb-2" />
          <p class="text-sm text-gray-600">dark</p>
        </div>
        <div class="text-center">
          <VMenu variant="primary" class="mb-2" />
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div class="text-center">
          <VMenu color="primary" class="mb-2" />
          <p class="text-sm text-gray-600">primary</p>
        </div>
        <div class="text-center">
          <VMenu color="secondary" class="mb-2" />
          <p class="text-sm text-gray-600">secondary</p>
        </div>
        <div class="text-center">
          <VMenu color="success" class="mb-2" />
          <p class="text-sm text-gray-600">success</p>
        </div>
        <div class="text-center">
          <VMenu color="warning" class="mb-2" />
          <p class="text-sm text-gray-600">warning</p>
        </div>
        <div class="text-center">
          <VMenu color="error" class="mb-2" />
          <p class="text-sm text-gray-600">error</p>
        </div>
        <div class="text-center">
          <VMenu color="info" class="mb-2" />
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <VMenu label="정상 상태" />
        <VMenu label="비활성화" disabled />
        <VMenu label="로딩 상태" loading />
        <VMenu label="에러 상태" :error="true" />
      </div>
    </div>

    <!-- 인터랙티브 데모 -->
    <template #interactive>
      <InteractiveDemo :props="demoProps" component-name="VMenu">
        <template #controls>
          <div class="space-y-4">
            <div>
...
import { ref } from 'vue'
import { VMenu } from '@/components/ui'
...
const MenuProps = [
  { name: 'label', type: 'string', default: '', description: 'Menu의 라벨' },
  { name: 'color', type: 'string', default: 'primary', description: 'Menu의 색상' },
  { name: 'size', type: 'string', default: 'default', description: 'Menu의 크기' },
...
const codeExample = 'VMenu 컴포넌트 사용 예시'
</script>
