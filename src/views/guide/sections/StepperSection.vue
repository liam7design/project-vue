  ...
  >
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div class="text-center">
          <VStepper variant="default" class="mb-2" />
          <p class="text-sm text-gray-600">default</p>
        </div>
        <div class="text-center">
          <VStepper variant="vertical" class="mb-2" />
          <p class="text-sm text-gray-600">vertical</p>
        </div>
        <div class="text-center">
          <VStepper variant="horizontal" class="mb-2" />
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <div class="text-center">
          <VStepper color="primary" class="mb-2" />
          <p class="text-sm text-gray-600">primary</p>
        </div>
        <div class="text-center">
          <VStepper color="secondary" class="mb-2" />
          <p class="text-sm text-gray-600">secondary</p>
        </div>
        <div class="text-center">
          <VStepper color="success" class="mb-2" />
          <p class="text-sm text-gray-600">success</p>
        </div>
        <div class="text-center">
          <VStepper color="warning" class="mb-2" />
          <p class="text-sm text-gray-600">warning</p>
        </div>
        <div class="text-center">
          <VStepper color="error" class="mb-2" />
          <p class="text-sm text-gray-600">error</p>
        </div>
        <div class="text-center">
          <VStepper color="info" class="mb-2" />
    ...
    <div class="mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <VStepper label="정상 상태" />
        <VStepper label="비활성화" disabled />
        <VStepper label="로딩 상태" loading />
        <VStepper label="에러 상태" :error="true" />
      </div>
    </div>

    <!-- 인터랙티브 데모 -->
    <template #interactive>
      <InteractiveDemo :props="demoProps" component-name="VStepper">
        <template #controls>
          <div class="space-y-4">
            <div>
...
import { ref } from 'vue'
import { VStepper } from '@/components/ui'
...
const StepperProps = [
  { name: 'label', type: 'string', default: '', description: 'Stepper의 라벨' },
  { name: 'color', type: 'string', default: 'primary', description: 'Stepper의 색상' },
  { name: 'size', type: 'string', default: 'default', description: 'Stepper의 크기' },
...
const codeExample = 'VStepper 컴포넌트 사용 예시'
</script>
